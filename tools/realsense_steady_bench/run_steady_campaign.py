...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "per-thread profile generated by generate_deadline_profile.py; "
            "required when policy deadline is selected"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "per-camera pre-DEADLINE warm-up deliveries; default is 10 percent "
            "of warm-up, capped at one second"
        ),
    )
    ...
    if args.deadline_profile is not None:
        if len(cases) != 1:
            raise SystemExit(
                "--deadline-profile can override exactly one selected case; "
        ...
        if not deadline_profile.is_file():
            raise SystemExit(f"DEADLINE profile does not exist: {deadline_profile}")
        for case in cases:
            case.setdefault("probe", {})["deadline_profile"] = str(deadline_profile)
    if "deadline" in args.policies:
        missing_profiles = [
            str(case["case_id"])
            for case in cases
        ...
        if missing_profiles:
            raise SystemExit(
                "policy deadline requires --deadline-profile or a per-case "
        ...
        for case in cases:
            ...
            if not profile.is_file():
                raise SystemExit(
                    f"DEADLINE profile for case {case['case_id']!r} does not exist: "
                    f"{profile}"
                )
